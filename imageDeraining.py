...
def psnr2(a,b):
	...
	MSE = abs(np.sum((np.array(a,np.int32)-np.array(b,np.int32))**2)/(256*256*3))
	print(MSE)
...
class Derain(object):
	...
	def __init__(self, data_dir,checkpoint_dir="./checkpoints1"):
		self.images = []
		self.dimension = []
		self.derainedImage = []
		self.rainedImage = []
...
if __name__=='__main__':
    # for training command 1 and 3 and 4
	# for testing command 1 and 2 and 5
	

	p = Derain("./training-data", "./checkpoints1")
	#p.load_model()
	p.train()	
	p.test()
    #p.sample_test()
